			...
			default :
				...
				<div class="comment-header">
					...
					<?php if ( $comment->user_id === $post->post_author ) : ?>
						<span class="comment-is-author label label-default"><?php _e( 'Autor', 'willow' ); ?></span>
					<?php endif; ?>
					<small class="comment-date">
						<?php printf( __( '%s | %s', 'willow' ), get_comment_date(), get_comment_time() ); ?>
					</small>
				</div>

				<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php _e( 'Seu comentário está aguardando moderação.', 'willow' ); ?></p>
				<?php endif; ?>
				...
				<div class="comment-action">
					<?php comment_reply_link( array_merge( $args, array(
						...
						'reply_text' => '<span class="btn btn-black btn-sm">' . __( 'Responder', 'willow' ) . '</span>',
						'depth'      => $depth,
